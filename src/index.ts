function levenshteinWithMax(
  ...
  for (let i = 1; i <= str1.length; i += 1) {
    ...
    const maxJ = str2.length > max + i ? max + i : str2.length
    current[0] = i
    ...
    for (let j = maxJ + 1; j <= str2.length; j += 1) {
      current[j] = big
    }
    if (colMin > max) {
 ...
 */
export function getSpellingSuggestion(
  name: string,
  candidates: string[],
): string | undefined
...
export function getSpellingSuggestion<T>(
  name: string,
  candidates: T[],
  getName: (candidate: T) => string | undefined = (a) => a as any,
): T | undefined {
